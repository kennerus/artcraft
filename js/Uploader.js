...
function Uploader() {
  ...
  this.init = function (options) {
    var defaultOptions = {
      maxCount: 8, // Максимальное кол-во объектов
      maxCountBox: null,
      maxSize: 5, // Максимальный размер mb.
      itemsCount: 0,
      itemsCountBox: null,
      multiple: false, // Множественная загрузка
      btnSelect: null, // Кнопка выбора файла
      btnCancel: null, // Кнопка отмены
      btnLoad: null, // Кнопка отправки на сервер
      fileInput: null, // Инпут загрузки
      itemContainer: null, // Контейнер в который будут загружаться объекты
      itemWrapper: null, // Шаблон для превью
      itemImg: null, // Контейнер в котором содержится превью (img)
      itemTitle: null, // Контейнер для названия файла
      itemSize: null, // Контейнер для размера файла
      uploadUrl: null, // URL для отправки запроса на сервер
      delItem: null, // Элемент при клике на который удаляется объект
      uploadItem: null, // Элемент при клике на который происходит загрузка файла
      dragNDrop: false, // Включение DragNDrop
      dropArea: null, // Область для перетаскивания файла
      filesExt: ['jpg', 'png'], // Разрешенные расширения файлов
      placeholder: 'img/placeholder.jpg', // Заглушка для превью
   ...
   */
  this.getExt = function (fileTitle) {
    var parts = fileTitle.split('.');
   ...
   * @returns {boolean}
   */
  this.isValidFile = function (fileExt) {
    return this.options.filesExt.indexOf(fileExt) !== -1;
  };

  this.renderItems = function (files) {
    this.filesCount = files.length;
    if (this.filesCount + this.itemsCount > this.options.maxCount) {
      console.log('Максимальное кол-во файлов');
    }
    else {
        ...
        reader.onload = (function (theFile) {
          return function (e) {
            if (theFile.size > this.options.maxSize * 1024 * 1024) {
              ...
              var currentFileExt = this.getExt(theFile.name);
              if (this.isValidFile(currentFileExt)) {
                this.renderItem({
                  title: theFile.name,
              ...
              // custom function after file added
              this.options.fileAdded(e);
              // Render thumbnail.
            }
          }.bind(this);
  ...
  this.renderItem = function (data) {
    ...
    if (img) {
      if (fileType[0] === 'image') {
        ...
        imgEl = document.createElement('img');
        imgEl.setAttribute('src', this.options.placeholder);
        img.appendChild(imgEl);
      }
    }
    if (size) {
    ...
    if (del) {
      del.onclick = function () {
        this.fileInput.value = null; // обнуление value инпута. каждый раз, когда добавляем файл - оно перезаписывается
  ...
  function setOptions(defaultOptions, options) {
    ...
    for (var key in options) {
      if (typeof options[key] === "object" && key !== "class" && !Array.isArray(options[key])) {
